...
public class ThreadPoolUtils {
     ...
     * @param builder
     */
    public ThreadPoolUtils(Builder builder) {
        // 构造有定时功能的线程池
//        ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(builder.mCorePoolSize, Integer.MAX_VALUE, KEEP_ALIVE_TIME, TimeUnit.MILLISECONDS, workQueue);
        scheduleExec = Executors.newScheduledThreadPool(builder.mCorePoolSize);
        switch (builder.mType) {
            case FixedThread:
                ...
                exec = Executors.newFixedThreadPool(builder.mCorePoolSize);
                break;
            case SingleThread:
    ...
    public static class Builder {//采用构造器模式
        private int mCorePoolSize = CORE_POOL_SIZE;
        private Type mType = Type.DefaultThread;
        ...
        public ThreadPoolUtils build() {
            return new ThreadPoolUtils(this);
        }
    }
