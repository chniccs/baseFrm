...
//                            "Got ACTION_MOVE event but don't have an active pointer id.");
                ...
                if (isChildScrollToBottom()) {
                    ...
                    if (mDisableWhenHorizontalMove && Math.abs(mInitialMotionX - x) > yDiff) {//这里主要是解决跟viewpager这类控件的冲突
                        return false;
                    }
                    if (yDiff > mTouchSlop && !mIsBeingDragged) {// 判断是否下拉的距离足够
    ...
    public void disableWhenHorizontalMove(boolean disableWhenHorizontalMove) {
        mDisableWhenHorizontalMove = disableWhenHorizontalMove;
    }
...
//                                "Got ACTION_UP event but don't have an active pointer id.");
    ...
    public class CircleProgressView extends View implements Runnable {
         ...
         * @return
         */
        private Paint createPaint(int index) {
            if (mColors == null || mColors.length == 0) {
                mColors = getResources().getIntArray(R.array.google_colors);
            }
            ...
            try {
                progressPaint.setColor(mColors[index % mColors.length]);
            } catch (Exception e) {
                e.printStackTrace();
            }
